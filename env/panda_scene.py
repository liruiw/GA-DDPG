...
def pairwise_link_collision(
    body1, link1, body2, link2=BASE_LINK, max_distance=MAX_DISTANCE, CLIENT=None
):  # 10000
    return (
        len(
            p.getClosestPoints(
...
def any_link_pair_collision(
    body1, body2, links1=None, links2=None, CLIENT=None, **kwargs
):
    if links1 is None:
...
class PandaTaskSpace6D:
    def __init__(self):
        self.high = np.array(
            [0.06, 0.06, 0.06, np.pi / 6, np.pi / 6, np.pi / 6]
        )  # , np.pi/10
        self.low = np.array(
            [-0.06, -0.06, -0.06, -np.pi / 6, -np.pi / 6, -np.pi / 6]
        )  # , -np.pi/3
...
class PandaYCBEnv:
    ...
    def reset(
        ...
        plane_file = os.path.join(
            self.root_dir, "data/objects/floor/model_normalized.urdf"
        )  # _white
        table_file = os.path.join(
            self.root_dir, "data/objects/table/models/model_normalized.urdf"
        )

        self.obj_path = [plane_file, table_file]
        self.plane_id = p.loadURDF(
            plane_file, [0 - self._shift[0], 0 - self._shift[1], -0.82 - self._shift[2]]
        )
        self.table_pos = np.array(
            [0.5 - self._shift[0], 0.0 - self._shift[1], -0.82 - self._shift[2]]
        )
        ...
        if scene_file is None or not os.path.exists(
            os.path.join(self.data_root_dir, scene_file + ".mat")
        ):
            self._randomly_place_objects(
                self._get_random_object(self._numObjects), scale=1
            )
        else:
    ...
    def step(self, action, delta=False, obs=True, repeat=None, config=False, vis=False):
        ...
        observation = self.input_selection(observation)
            ...
            depth.copy(), mask, use_depth_heuristics=self._termination_heuristics
        )
        self.collision_check()
        reward = self._reward()

        info = {
            "grasp_success": reward,
    ...
    def _get_observation(self, pose=None, vis=False, acc=True):
        ...
        object_pose = self._get_target_relative_pose(
            "ef"
        )  # self._get_relative_ef_pose()
        ...
        (
            ...
            near,
            far,
        ) = self._get_hand_camera_view(pose)
        ...
        depth = (far * near / (far - (far - near) * depth) * 5000).astype(
            np.uint16
        ...
        obs = self.process_image(
            obs[..., :3], obs[..., [3]], obs[..., [4]], tuple(self._resize_img_size)
        )
        intrinsic_matrix = projection_to_intrinsics(
            hand_proj_matrix, self._window_width, self._window_height
        )
        point_state = backproject_camera_target(
            obs[3].T, intrinsic_matrix, obs[4].T
        )  # obs[4].T

        point_state = (
            self.cam_offset[:3, :3].dot(point_state) + self.cam_offset[:3, [3]]
        )
        point_state[1] *= -1
    ...
    def _termination(self, depth_img, mask_img, use_depth_heuristics=False):
        ...
        if use_depth_heuristics:
            ...
            # hard coded region
            depth_img_roi = depth_img[
                int(38.0 * self._window_height / 64) :,
                int(24.0 * self._window_width / 64) : int(48 * self._window_width / 64),
            ]
        ...
        return (
            self._env_step >= self._maxSteps
            or depth_heuristics
            or self.target_fall_down()
        )
    ...
    def cache_objects(self):
        ...
        self.target_obj_indexes = [
            self._all_obj.index(idx) for idx in self._target_objs
        ]
        ...
        for i, name in enumerate(objects_paths):
            ...
            uid = self._add_mesh(
                os.path.join(self.root_dir, name, "model_normalized.urdf"), trans, orn
            )  # xyzw
            ...
            point_z = np.loadtxt(
                os.path.join(self.root_dir, name, "model_normalized.extent.txt")
            )
    ...
    def cache_reset(self, scene_file, init_joints, enforce_face_target):
        ...
        observation = (
            self.enforce_face_target()
            if enforce_face_target
    ...
    def place_objects_from_scene(self, scene_file, objectUids=None):
        ...
        print(
            "==== loaded scene: {} target: {} idx: {} init joint".format(
                scene_file.split("/")[-1], self.target_name, self.target_idx
            )
        )
    ...
    def load_textures(self):
        ...
        table_textures = [
            p.loadTexture(os.path.join(texture_dir, f)) for f in random_files
        ]
    ...
    def update_curr_acc_points(self, new_points):
        ...
        index = np.random.choice(
            ...
            size=int(self._pt_accumulate_ratio**self._env_step * new_points.shape[1]),
            replace=False,
        ).astype(np.int)
        self.curr_acc_points = np.concatenate(
            (new_points[:, index], self.curr_acc_points), axis=1
        )  #
    ...
    def process_action(self, action, delta=False, config=False):
        ...
        if config:
            ...
            jointPoses = np.array(
                p.calculateInverseKinematics(
                    self._panda.pandaUid, self._panda.pandaEndEffectorIndex, pos, orn
                )
            )
            jointPoses[-2:] = 0.04
            action = jointPoses
    ...
    def _sample_ef(self, target, near=0.35, far=0.50):
        ...
        for _ in range(outer_loop_num):
            ...
            r = np.random.uniform(
                low=self._initial_near, high=self._initial_far
            )  # sphere radius
            pos = np.array(
                [
            ...
            for i in range(inner_loop_num):
                ...
                ik = self.robot.inverse_kinematics(
                    trans, quat, seed=anchor_seeds[np.random.randint(len(anchor_seeds))]
                )  # , quat
                if ik is not None:
                    break
        return ik
    ...
    def _get_hand_camera_view(self, cam_pose=None):
        ...
        hand_near = 0.035
        hand_far = 2
        hand_proj_matrix = p.computeProjectionMatrixFOV(
            fov, aspect, hand_near, hand_far
        )
        hand_cam_view_matrix = se3_inverse(
            cam_pose_mat.dot(rotX(-np.pi / 2).dot(rotZ(-np.pi)))
        ).T  # z backward
    ...
    def setup_expert_scene(self):
        ...
        for i, name in enumerate(self.obj_path[:-2]):
            ...
            self.planner_scene.env.add_object(
                name, trans, tf_quat(orn), compute_grasp=True
            )

        self.planner_scene.env.add_plane(
            np.array([0.05, 0, -0.17]), np.array([1, 0, 0, 0])
        )  # never moved
        self.planner_scene.env.add_table(
            np.array([0.55, 0, -0.17]), np.array([0.707, 0.707, 0.0, 0])
        )
    ...
    def expert_plan(self, step=-1, return_success=False):
        ...
        for i, name in enumerate(object_lists[:-2]):  # for this scene
            ...
            idx = self.obj_path[:-2].index(
                os.path.join(self.root_dir, "data/objects/" + name + "/")
            )
            exists_ids.append(idx)
            trans, orn = self.placed_object_poses[idx]
            placed_poses.append(np.hstack([trans, ros_quat(orn)]))

        planner_cfg.disable_collision_set = [
            name.split("/")[-2]
            for idx, name in enumerate(self.obj_path[:-2])
            if idx not in exists_ids
        ]

        joint_pos = self._panda.getJointStates()[0]
        self.planner_scene.traj.start = np.array(joint_pos)
        self.planner_scene.env.set_target(
            self.obj_path[self.target_idx].split("/")[-2]
        )  # scene.env.names[0])

        if step > 0:  # plan length
            self.planner_scene.env.objects[
                self.planner_scene.env.target_idx
            ].compute_grasp = False
        ...
        ef_pose = unpack_pose(base_pose).dot(
            self.robot.forward_kinematics_parallel(
                wrap_value(plan[-1])[None], offset=False
            )[0][-3]
        )  # world coordinate
    ...
    def _randomly_place_objects(self, urdfList, scale, poses=None):
        ...
        x_rot = 0
        z_init = -0.65 + 2 * self.object_heights[self.target_idx]
        ...
        pos, new_orn = p.getBasePositionAndOrientation(
            self._objectUids[self.target_idx]
        )  # to target
        ang = (
        ...
        if (
            self.target_name in self._filter_objects or ang > 50
        ):  # self.target_name.startswith('0') and
    ...
    def _get_random_object(self, num_objects):
        ...
        selected_objects_filenames = [
            os.path.join(
                "data/objects/",
                self.obj_indexes[int(selected_objects[0])],
    ...
    def collision_check(self):
        ...
        if any_link_pair_collision(
            self._objectUids[self.target_idx], self._panda.pandaUid, CLIENT=self.cid
        ):
            if (
                self._accumulate_points
                and self.curr_acc_points.shape[1] > self._uniform_num_pts
            ):  # touch the target object
                self.curr_acc_points = regularize_pc_point_count(
                    self.curr_acc_points.T, self._uniform_num_pts
                ).T
            self.collided = True
    ...
    def get_env_info(self, scene_file=None):
        ...
        for idx, uid in enumerate(self._objectUids):
            if self.placed_objects[idx] or idx >= len(self._objectUids) - 2:
                ...
                obj_dir.append(
                    "/".join(self.obj_path[idx].split("/")[:-1]).strip()
                )  # .encode("utf-8")

        return obj_dir, poses

    def convert_action_from_joint_to_cartesian(
        self, joints, joint_old=None, delta=False
    ):
        """
        ...
        ef_pose = self.robot.forward_kinematics_parallel(
            wrap_value(joint_old)[None], offset=False
        )[0][-3]
        pos, rot = ef_pose[:3, 3], ef_pose[:3, :3]
        ef_pose_ = self.robot.forward_kinematics_parallel(
            wrap_value(joints)[None], offset=False
        )[0][-3]
    ...
    def process_pointcloud(
        self, point_state, vis, acc_pt=True, use_farthest_point=False
    ):
        """
        ...
        if self._regularize_pc_point_count and point_state.shape[1] > 0:
            point_state = regularize_pc_point_count(
                point_state.T, self._uniform_num_pts, use_farthest_point
            ).T
    ...
    def _get_goal_dist(self):
        ...
        if hasattr(self, "cur_goal"):
            ...
            goal_control_point = (
                goal_pose[:3, :3].dot(self.hand_finger_points) + goal_pose[:3, [3]]
            )
    ...
    def _get_nearest_goal_pose(self, rotz=False, mat=False):
        ...
        obj_pose = list(pos) + [orn[3], orn[0], orn[1], orn[2]]
        ws_goal_set = self.planner_scene.env.objects[
            self.planner_scene.env.target_idx
        ].grasps_poses
        ...
        point_2 = (
            np.matmul(rel_pose[:, :3, :3], self.hand_finger_points[None])
            + rel_pose[:, :3, [3]]
        )
        ...
        if mat:
            return (
                inv_relative_pose(cur_goal, ef_pose).dot(rotZ(np.pi / 2))
                if rotz
                else inv_relative_pose(cur_goal, ef_pose)
            )
        if rotz:
            return pack_pose_rot_first(
                inv_relative_pose(cur_goal, ef_pose).dot(rotZ(np.pi / 2))
            )
    ...
    def _get_relative_goal_pose(self, rotz=False, mat=False, nearest=False):
        ...
        return pack_pose_rot_first(
            inv_relative_pose(cur_goal, ef_pose)
        )  # to be compatible with graspnet
