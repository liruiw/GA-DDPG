...
class PointCloudReader:
    ...
    def change_object(self, cad_path, cad_scale):
        if not os.path.exists(cad_path):
            # clemen directory
            path = cad_path
            key_str = 'NVGraspDataset_clemens_meshes_'
            start_idx = path.find(key_str)
            new_mesh_dir, mesh_folder = path[start_idx + len(key_str):].split('_')
            path = os.path.join('/home/liruiw/Projects/OOMG/data/NVGraspDataset/meshes', new_mesh_dir, mesh_folder)
            print(path)
            cad_path = path
        self._renderer.change_object(cad_path, cad_scale)

    def get_evaluator_data(self, grasp_path, verify_grasps=False, transform=True):
        ...
        for iter in range(self._batch_size):
            if iter > 0:
                ...
                pc, camera_pose, _ = self.render_random_scene(camera_pose = np.eye(4))
                output_pcs.append(pc)
                output_pc_poses.append(inverse_transform(camera_pose))

            if transform:  output_grasps[iter] = camera_pose.dot(output_grasps[iter]) # just want the set
        ...
        return output_pcs, output_grasps, output_labels, output_qualities, output_pc_poses, output_cad_paths, output_cad_scales, obj_mesh


    def _get_uniform_evaluator_data(self, grasp_path, verify_grasps=True):
        pos_grasps, pos_qualities, neg_grasps, neg_qualities, obj_mesh, cad_path, cad_scale = self.read_grasp_file(grasp_path)
    ...
    def read_object_grasp_data(
        ...
        object_model = sample.Object(os.path.join(root_folder, path))
...
if __name__ == '__main__':
    ...
    grasp_paths = [g for g in grasp_paths if 'Bottle' not in g and 'mug' not in g and 'Bowl' not in g]
