...
import cv2
import sys
sys.path.insert(0, 'pybullet-blender-recorder')
...
def test(run_iter=0):
    ...
    episodes = 11 # args.test_episode_num
    k = 10
    ...
    for idx, path in zip(env._objectUids, env.obj_path):
        if 'floor' in path: continue
        if 'table' in path:
            recorder.register_object(idx,  path  )
        else:
            recorder.register_object(idx,  path + 'model_normalized.urdf'   )
    recorder.register_object(env._panda.pandaUid,  'env/models/'+ env._panda.path.replace('.urdf', '_blender.urdf')    )
    while (k < episodes):
    ...
    if run_iter == NUM_RUNS - 1 and args.log:
        ...
        print('testing script:', args.output_file)
    recorder.save('demo_{}.pkl'.format(k))
